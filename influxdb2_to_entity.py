...
class InfluxDB2_to_entity(hass.Hass):
        ...
        if CONF_ENTITYSETTINGS in self.args:
            ...
            for entityID, entity_settings in self.args[CONF_ENTITYSETTINGS][CONF_ENTITIES].items():
                if  not isinstance(entityID,str) or not isinstance(entity_settings,dict) \
                    ...
                    self.run_daily(self.run_daily_update_entity,startTime, ent = entity, entset = entity_settings)
    ...
    def update_influx_entity(self, entityID, entity_settings):
        ...
        _query = self.build_influx2_query(_bucket,_rangeStart,_rangeStop,_query,_imports,_options,_group)
    ...
    def build_influx2_query(self,bucket,range_start,range_stop,query,imports,options,group):
        query_prefix = f'from(bucket:"{bucket}") |> range(start: {range_start}, stop: {range_stop}) |>'
